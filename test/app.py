...
def execute_mismatch_explorer_job(run_id, params, unix_config):
    ...
    try:
        ...
        if exit_code == 0:
            run_registry[run_id]['status'] = 'SUCCESS'
            add_log(run_id, "✅ Execution completed successfully")
        else:
            run_registry[run_id]['status'] = 'FAILED'
...
def mismatch_explorer_fetch(run_id):
    ...
    try:
        ...
        add_log(run_id, "📥 Fetching results from Unix...")
        ...
        if not unix_config.get('host'):
            add_log(run_id, "❌ No Unix configuration available")
            return jsonify({'error': 'No Unix configuration available'}), 400
        
        add_log(run_id, f"📡 Reading report file: /tmp/{run_id}_record_explorer.json")
        ...
        try:
            ...
            add_log(run_id, "✅ SSH connected successfully for fetch")
        except Exception as conn_error:
            add_log(run_id, f"⚠️ SSH connection failed during fetch: {str(conn_error)}")
            ...
            add_log(run_id, "✅ Using fallback mock data")
            return jsonify({
                'jobId': job_id,
                'status': 'completed',
                'totalRecords': len(summary_rows),
        ...
        try:
            ...
            add_log(run_id, f"✅ File read successfully ({len(json_content)} bytes)")
            records = json.loads(json_content)
        except FileNotFoundError:
            add_log(run_id, f"⚠️ Primary file not found: {remote_path}, trying alternatives...")
            ...
            if records is None:
                add_log(run_id, "❌ No report files found, using mock data")
            ...
            add_log(run_id, f"❌ Error reading file: {str(read_error)}")
            sftp.close()
            ssh_client.close()
        ...
        add_log(run_id, f"📊 Detected format: {debug_info.get('detected_format', 'unknown')}")
        add_log(run_id, f"📊 Raw record count: {debug_info.get('raw_count', 0)}")
        add_log(run_id, f"📊 Parsed record count: {debug_info.get('parsed_count', 0)}")
        if debug_info.get('skipped_count', 0) > 0:
            add_log(run_id, f"⚠️ Skipped invalid records: {debug_info.get('skipped_count', 0)}")
        for msg in debug_info.get('messages', []):
            add_log(run_id, f"  ℹ️ {msg}")
        ...
        add_log(run_id, f"✅ Results ready: {len(summary_rows)} record(s) loaded")
