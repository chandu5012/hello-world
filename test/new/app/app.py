...
# ============================================================
# DOWNLOAD CACHE - Local file cache for OTP-safe downloads
# ============================================================
DOWNLOAD_CACHE_DIR = os.path.join(tempfile.gettempdir(), 'lovable_download_cache')
...
# {
#   run_id: {
#       'local_path': str,        # Full local path to cached file (single) or ZIP
#       'download_name': str,     # Filename for download
#       'tab': str,               # Which tab produced it
#       'cached_at': datetime,
#       'files': list,            # (optional) list of {'local_path', 'name'} for multi-file
#       'is_zip': bool            # True if download is a ZIP of multiple files
#   }
# }
...
def determine_download_filename(params, run_id=None):
    """
    Determine the output filename for single_table comparison based on comparison type.
    
    If run_id is provided, filename is prefixed with run_id_.
    ...
    if comparison_type == 'database_to_database':
        if target_table:
            base_filename = f'{target_table}.csv'
        else:
            base_filename = 'comparison_output.csv'
        ...
        base_filename = 'file_to_file.csv'
    elif comparison_type == 'database_to_file':
        base_filename = 'db_to_file.csv'
    else:
        base_filename = 'comparison_output.csv'
    
    # Prefix with run_id if provided
    if run_id:
        filename = f'{run_id}_{base_filename}'
    else:
        filename = base_filename
    
    unix_path = f'/tmp/{filename}'
...
def fetch_and_cache_output_file(ssh_client, run_id, remote_path, download_name, tab):
    """
    Fetch output file from Unix server via SFTP and cache locally.
    Must be called BEFORE closing SSH connection.
    
    Returns True if successful, False otherwise.
    """
    try:
        add_log(run_id, f'✅ Script execution completed (rc=0). Fetching {remote_path}...')
        ...
        for attempt in range(1, 4):
            try:
                ...
                add_log(run_id, f'Waiting for output file... attempt {attempt}/3')
                time.sleep(1)
        
        if not file_found:
            # Try without run_id prefix as fallback
            alt_paths = []
            if run_id and run_id in remote_path:
                # Try path without run_id prefix
                base_name = remote_path.replace(f'/tmp/{run_id}_', '/tmp/')
                alt_paths.append(base_name)
            alt_paths.extend(['/tmp/output.csv', f'/tmp/{download_name}'])
            ...
            for alt_path in alt_paths:
                try:
                    ...
                    file_found = True
                    add_log(run_id, f'✅ Found file at alternative path: {alt_path}')
                    break
                except FileNotFoundError:
                    continue
        
        if not file_found:
            add_log(run_id, f'❌ Output file not found at {remote_path} (tried alternatives)')
            sftp.close()
            return False
        
        # Create local cache directory for this run
        run_cache_dir = os.path.join(DOWNLOAD_CACHE_DIR, run_id)
        os.makedirs(run_cache_dir, exist_ok=True)
        
        local_path = os.path.join(run_cache_dir, download_name)
        
        # Download file
        sftp.get(remote_path, local_path)
        sftp.close()
        
        # Register in download cache
        download_registry[run_id] = {
        ...
        add_log(run_id, f'✅ File cached for download (no SSH needed). Size: {file_size} bytes')
        
        return True
        
    except Exception as e:
        add_log(run_id, f'❌ Failed to fetch/cache output file: {str(e)}')
        return False


def fetch_and_cache_run_id_files(ssh_client, run_id, tab):
    """
    Discover and fetch ALL files matching /tmp/{run_id}_* via SFTP.
    Used for Data Comparison single-mode where multiple output files may exist.
    If multiple files found, creates a ZIP. Returns True if at least one file cached.
    """
    import zipfile
    try:
        add_log(run_id, f'✅ Script execution completed (rc=0). Discovering /tmp/{run_id}_* files...')
        
        sftp = ssh_client.open_sftp()
        
        # List /tmp and find matching files with retries
        matching_files = []
        for attempt in range(1, 4):
            try:
                all_files = sftp.listdir('/tmp')
                prefix = f'{run_id}_'
                ...
                pass
            add_log(run_id, f'Waiting for output files... attempt {attempt}/3')
            time.sleep(1)
        
        if not matching_files:
            add_log(run_id, f'❌ No output files found in /tmp for run_id: {run_id}')
            sftp.close()
            return False
        
        add_log(run_id, f'📄 Found {len(matching_files)} output file(s): {", ".join(matching_files)}')
        ...
        for fname in matching_files:
            ...
            local_path = os.path.join(run_cache_dir, fname)
            try:
                sftp.get(remote_path, local_path)
                cached_files.append({'local_path': local_path, 'name': fname})
                file_size = os.path.getsize(local_path)
                add_log(run_id, f'  ✅ Fetched {fname} ({file_size} bytes)')
            except Exception as e:
                add_log(run_id, f'  ❌ Failed to fetch {fname}: {str(e)}')
        
        sftp.close()
        
        if not cached_files:
            add_log(run_id, f'❌ Could not fetch any output files for run_id: {run_id}')
            return False
        
        if len(cached_files) == 1:
            # Single file - serve directly
            download_registry[run_id] = {
                'local_path': cached_files[0]['local_path'],
                'download_name': cached_files[0]['name'],
                'tab': tab,
                'cached_at': datetime.now(),
                'files': cached_files,
                'is_zip': False
            }
            add_log(run_id, f'✅ File cached for download (no SSH needed).')
        else:
            # Multiple files - create ZIP
            zip_name = f'{run_id}_data_comparison_outputs.zip'
            zip_path = os.path.join(run_cache_dir, zip_name)
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                for cf in cached_files:
                    zf.write(cf['local_path'], cf['name'])
            ...
            add_log(run_id, f'✅ {len(cached_files)} files zipped into {zip_name} ({zip_size} bytes). Download ready (no SSH needed).')
        
        return True
        
    except Exception as e:
        add_log(run_id, f'❌ Failed to discover/fetch output files: {str(e)}')
        return False
...
def get_unix_config_from_params(params):
    ...
    # Extract Cluster Type (NEW)
    cluster_type = (
        params.get('cluster_type') or
        params.get('clusterType') or
        params.get('comparisonClusterType') or
        params.get('loadClusterType') or
        params.get('schemaClusterType') or
        params.get('fileloadClusterType') or
        params.get('filedownloadClusterType') or
        params.get('mismatchClusterType') or
        'dpc'
    ).strip()
    ...
    return {
        ...
        'cluster_type': cluster_type  # NEW: cluster type (dpc/mapr/custom)
    }
...
# ============================================================
# SINGLE TABLE COMPARISON - FILE DOWNLOAD ENDPOINT (CACHED)
# ============================================================
@app.route('/api/static/comparison/download/<run_id>')
def static_comparison_download(run_id):
    """
    Download the comparison output file from local cache (OTP-safe).
    Falls back to SSH fetch if cache miss.
    """
    return serve_cached_download(run_id, 'data_comparison')


# ============================================================
# GENERIC CACHED DOWNLOAD ENDPOINT
# ============================================================
@app.route('/api/static/cached-download/<run_id>')
def static_cached_download(run_id):
    """
    Generic cached download endpoint. Serves any cached output file by run_id.
    No SSH required - file was fetched immediately after execution.
    """
    return serve_cached_download(run_id)
...
def _determine_mimetype(filename):
    ...
    elif dl_lower.endswith('.xml'):
        return 'application/xml'
    elif dl_lower.endswith('.sql') or dl_lower.endswith('.txt'):
        return 'text/plain'
...
def serve_cached_download(run_id, expected_tab=None):
    ...
    Supports single files and ZIP bundles (for multi-file Data Comparison output).
    """
    try:
        if run_id not in run_registry:
            return jsonify({'error': 'Run ID not found'}), 404
        ...
        if expected_tab and run_entry['tab'] != expected_tab:
            return jsonify({'error': f'Download is only available for {expected_tab} runs'}), 400
        ...
        # Try to serve from local cache (OTP-safe)
...
def execute_filedownload_with_upload(run_id, params, file_content, original_filename, unix_dest_path):
    ...
    try:
        ...
        # Add run_id to params for Unix script
        params['run_id'] = run_id
        add_log(run_id, f'🆔 run_id generated: {run_id}')
        ...
        # Fetch & cache output BEFORE closing SSH (OTP-safe)
        if success and not run_registry[run_id]['stop_requested']:
            target_file_name = params.get('target_file_name', 'output.csv')
            download_name = f'{run_id}_{target_file_name}'
            remote_path = f'/tmp/{download_name}'
            
            cached = fetch_and_cache_output_file(ssh_client, run_id, remote_path, download_name, 'file_download')
            
            run_registry[run_id]['download_available'] = cached
            run_registry[run_id]['download_filename'] = download_name
            run_registry[run_id]['unix_output_path'] = remote_path
        
        # Close SSH connection AFTER fetching
        ssh_client.close()
        ...
        if run_registry[run_id]['stop_requested']:
            ...
            if run_registry[run_id].get('download_available'):
                add_log(run_id, f'⬇️ Output file ready for download: {run_registry[run_id].get("download_filename")}')
        else:
            run_registry[run_id]['status'] = 'FAILED'
...
def static_filedownload_download(run_id):
    """Download the generated file for File Download tab (from local cache, OTP-safe)."""
...
def execute_schema_with_upload(run_id, params, file_content, original_filename, unix_dest_path):
    ...
    try:
        ...
        # Fetch & cache output BEFORE closing SSH (OTP-safe)
        if success and not run_registry[run_id]['stop_requested']:
            base_output_name = params.get('output_file_name', 'schema_output.sql')
            download_name = f'{run_id}_{base_output_name}'
            remote_path = f'/tmp/{download_name}'
            
            cached = fetch_and_cache_output_file(ssh_client, run_id, remote_path, download_name, 'schema_generation')
...
def execute_static_operation(run_id, tab, params):
    ...
    try:
        ...
        # Add run_id to params so it gets passed to Unix script
        params['run_id'] = run_id
        ...
        # Build script arguments (run_id is already in params)
        script_args = build_script_arguments(params)
        ...
        # --- SINGLE MODE: Fetch & cache output file BEFORE closing SSH ---
        if success and not run_registry[run_id]['stop_requested']:
            # For single_table data_comparison, discover & fetch all run_id_* files
            if tab == 'data_comparison' and comparison_mode == 'single_table':
                cached = fetch_and_cache_run_id_files(ssh_client, run_id, tab)
                
                run_registry[run_id]['download_available'] = cached
                if cached and run_id in download_registry:
                    run_registry[run_id]['download_filename'] = download_registry[run_id]['download_name']
            
            # For file_download tab, fetch output immediately
            elif tab == 'file_download':
                download_fields = extract_file_download_fields(params)
                base_target_name = download_fields['target_file_name']
                download_name = f'{run_id}_{base_target_name}'
                remote_path = f'/tmp/{download_name}'
                
                cached = fetch_and_cache_output_file(ssh_client, run_id, remote_path, download_name, tab)
                
                run_registry[run_id]['download_available'] = cached
                run_registry[run_id]['download_filename'] = download_name
                run_registry[run_id]['unix_output_path'] = remote_path
            
            # For schema_generation tab, fetch output immediately
            elif tab == 'schema_generation':
                base_output_name = (
        ...
        # Close SSH connection AFTER fetching output files
        ssh_client.close()
        run_registry[run_id]['ssh_client'] = None
        add_log(run_id, '🔌 Unix server connection closed')
        ...
        if run_registry[run_id]['stop_requested']:
            ...
            # Log download availability
            if run_registry[run_id].get('download_available'):
                add_log(run_id, f'⬇️ Output file ready for download: {run_registry[run_id].get("download_filename", "output")}')
        else:
            run_registry[run_id]['status'] = 'FAILED'
...
def execute_mismatch_explorer_job(run_id, params, unix_config):
    ...
    try:
        ...
        command += f" --cluster_type={shlex.quote(unix_config.get('cluster_type', 'dpc').strip())}"
        command += f" --user_mail={shlex.quote(unix_config.get('user_email', '').strip())}"
        command += f" --batch_id={shlex.quote(unix_config['batch_id'].strip())}"
        command += f" --unix_user={shlex.quote(unix_config['username'].strip())}"
        command += f" --unix_password={shlex.quote(unix_config['password'].strip())}"
        
        # Log Unix params confirmation (never log password)
        add_log(run_id, f"Mismatch Explorer Unix params: unix_host={unix_config['host'].strip()}, cluster_type={unix_config.get('cluster_type', 'dpc').strip()}, batch_id={unix_config['batch_id'].strip()}, unix_user={unix_config['username'].strip()}")
