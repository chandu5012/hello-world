   ...
   return `
         <div class="unix-server-section mb-6">
             <div class="source-target-card border-t-4 border-t-wellsfargo-red shadow-elevated">
                ...
                <div class="config-content p-6 space-y-4">
                    <!-- Row 1: Unix Host + Cluster Type -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="form-group">
                            <label for="${prefix}UnixHost" class="block text-sm font-medium mb-2">🖥️ Unix Host</label>
                            <select id="${prefix}UnixHost" class="form-select w-full p-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200" onchange="handleUnixHostDropdownChange('${prefix}')">
                                <option value="1noc.oemma-dev.abc.net" selected>1noc.oemma-dev.abc.net</option>
                                <option value="1noc.oemma-sit.abc.net">1noc.oemma-sit.abc.net</option>
                                <option value="1noc.oemma-uat.abc.net">1noc.oemma-uat.abc.net</option>
                                <option value="__manual__">Enter Manually</option>
                            </select>
                            <div id="${prefix}UnixHostManualContainer" class="mt-2 hidden">
                                <input type="text" id="${prefix}UnixHostManual" class="form-input w-full p-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200" placeholder="Enter custom Unix host" maxlength="255">
                            </div>
                        </div>
                        
                        <div class="form-group">
                            <label for="${prefix}ClusterType" class="block text-sm font-medium mb-2">🔧 Cluster Type</label>
                            <select id="${prefix}ClusterType" class="form-select w-full p-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200" onchange="handleClusterTypeDropdownChange('${prefix}')">
                                <option value="dpc" selected>dpc</option>
                                <option value="mapr">mapr</option>
                                <option value="__manual__">Enter Manually</option>
                            </select>
                            <div id="${prefix}ClusterTypeManualContainer" class="mt-2 hidden">
                                <input type="text" id="${prefix}ClusterTypeManual" class="form-input w-full p-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200" placeholder="Enter custom cluster type" maxlength="100">
                            </div>
                        </div>
                    </div>
                    
                    <!-- Row 2: User Email + Batch ID -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="form-group">
                    ...
                    <!-- Row 3: Unix Username + Unix Password -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="form-group">
...
// Save Unix config values for a prefix before form regeneration
function saveUnixConfig(prefix) {
    const config = {};
    const fields = ['UnixHost', 'UnixHostManual', 'ClusterType', 'ClusterTypeManual',
                     'UserEmail', 'BatchId', 'UnixUsername', 'UnixPassword'];
    fields.forEach(f => {
...
// Restore Unix config values for a prefix after form regeneration
function restoreUnixConfig(prefix, config) {
    if (!config) return;
    const fields = ['UnixHost', 'UnixHostManual', 'ClusterType', 'ClusterTypeManual',
                     'UserEmail', 'BatchId', 'UnixUsername', 'UnixPassword'];
    fields.forEach(f => {
    ...
    // Show/hide manual containers based on restored values
    handleUnixHostDropdownChange(prefix);
    handleClusterTypeDropdownChange(prefix);
}

// Handle Unix Host dropdown change (show/hide manual input)
function handleUnixHostDropdownChange(prefix) {
    const select = document.getElementById(prefix + 'UnixHost');
    const manualContainer = document.getElementById(prefix + 'UnixHostManualContainer');
    if (!select || !manualContainer) return;
...
// Handle Cluster Type dropdown change (show/hide manual input)
function handleClusterTypeDropdownChange(prefix) {
    const select = document.getElementById(prefix + 'ClusterType');
    const manualContainer = document.getElementById(prefix + 'ClusterTypeManualContainer');
    if (!select || !manualContainer) return;
    if (select.value === '__manual__') {
        manualContainer.classList.remove('hidden');
    } else {
...
// Resolve Unix Host final value (dropdown or manual)
function resolveUnixHostValue(prefix) {
    const select = document.getElementById(prefix + 'UnixHost');
    if (!select) return '';
    if (select.value === '__manual__') {
        const manual = document.getElementById(prefix + 'UnixHostManual');
        return manual ? manual.value.trim() : '';
    }
    return select.value;
}

// Resolve Cluster Type final value (dropdown or manual)
function resolveClusterTypeValue(prefix) {
    const select = document.getElementById(prefix + 'ClusterType');
    if (!select) return 'dpc';
    if (select.value === '__manual__') {
        const manual = document.getElementById(prefix + 'ClusterTypeManual');
        return manual ? manual.value.trim() : '';
    }
    return select.value;
}
...
function handleTableModeChange(mode) {
  ...
  if (container) {
    // Save Unix config before regeneration
    const savedUnix = saveUnixConfig("comparison");
    ...
    // Restore Unix config after regeneration
    restoreUnixConfig("comparison", savedUnix);
  }
}
...
function collectFileDownloadFormData(sourceType) {
  ...
  // Unix server fields
  data.filedownloadUnixHost = resolveUnixHostValue("filedownload");
  data.filedownloadClusterType = resolveClusterTypeValue("filedownload");
...
function collectFormData(tabPrefix) {
  ...
  // Resolve Unix Host dropdown (all tabs)
  // The select IDs follow pattern: {prefix}UnixHost
  const prefixes = ["comparison", "load", "schema", "fileload", "filedownload"];
  for (const pfx of prefixes) {
    const hostKey = pfx + "UnixHost";
    if (data[hostKey] === "__manual__") {
      const manualKey = pfx + "UnixHostManual";
      data[hostKey] = data[manualKey] ? data[manualKey].trim() : "";
    }
    // Resolve Cluster Type
    const clusterKey = pfx + "ClusterType";
    if (data[clusterKey] === "__manual__") {
      const manualKey = pfx + "ClusterTypeManual";
      data[clusterKey] = data[manualKey] ? data[manualKey].trim() : "";
    }
  }
...
function collectMismatchFormData() {
  return {
    ...
    // Unix configuration
    mismatchUnixHost: resolveUnixHostValue('mismatch'),
    mismatchClusterType: resolveClusterTypeValue('mismatch'),
...
function startMismatchLogStreaming(runId) {
  ...
  eventSource.onmessage = function(event) {
    try {
      ...
      switch (data.type) {
        ...
        case 'status':
          updateLogStatusRing(data.status);
          // If terminal status, check for finalization in 'end'
          break;
          
        case 'end':
          eventSource.close();
          eventSource = null;
          finalizeMismatchExecution(data.status || 'FAILED', data.status === 'SUCCESS');
          // Stop polling since SSE delivered final status
        ...
        case 'error':
          ...
          eventSource.close();
          eventSource = null;
          finalizeMismatchExecution('FAILED', false);
          if (mismatchPollTimer) {
            clearInterval(mismatchPollTimer);
            mismatchPollTimer = null;
          }
  ...
  eventSource.onerror = function(error) {
    ...
    // Fallback: start polling to get final status
    startMismatchStatusPolling(runId);
  };
  
  // Safety polling in parallel with SSE
  startMismatchStatusPolling(runId);
}

/**
 * Poll /api/mismatch-explorer/status every 2s until done
 */
let mismatchPollTimer = null;
function startMismatchStatusPolling(runId) {
  if (mismatchPollTimer) return; // already polling
  mismatchPollTimer = setInterval(async () => {
    try {
      const resp = await fetch(`/api/mismatch-explorer/status?runId=${runId}`);
      if (!resp.ok) return;
      const data = await resp.json();
      if (!data.isRunning) {
        clearInterval(mismatchPollTimer);
        mismatchPollTimer = null;
        finalizeMismatchExecution(data.status, data.hasResults);
...
/**
 * Finalize Mismatch Explorer UI after execution completes
 */
function finalizeMismatchExecution(status, hasResults) {
  // Prevent double-finalize
  if (mismatchExecutionStatus === status) return;
  ...
  if (status === 'SUCCESS') {
    ...
    if (fetchBtnText) fetchBtnText.textContent = 'Fetch & Compare';
  } else {
    mismatchExecutionStatus = 'FAILED';
    updateLogStatusRing('FAILED');
    fetchBtn.disabled = !hasResults;
  }
